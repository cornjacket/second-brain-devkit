...
| `tools/check_mcp_server.py` | Opt-in behavioral MCP check — drives the emitted stdio server (test backend); asserts the four-tool surface, **no `outputSchema`**, search + glossary tools, and the #21 negative suite (traversal refusals, substrate disjointness, glossary embedding-free — driven against a second brain whose vector cache is poisoned). SKIP when `mcp` absent. |
