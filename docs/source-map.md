...
| `tools/update_brain.py` | **Lifecycle (G4)** — non-destructively upgrade an existing brain's tooling from `template/` (dry-run; `--apply` commits). Preserves `vault/`/`data/`/`config/` + history, and the user's space outside the `README.md`/`CLAUDE.md` managed markers, which it **splices** rather than clobbers (#9, #40). `--adopt` brings either file, when marker-less, under management — keeping the old file verbatim below the block. Reports — never writes — a `vault/templates/new-note.md` that has fallen behind the emitted seed. |
