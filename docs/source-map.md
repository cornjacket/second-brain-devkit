...
| `note_view.py` | The **canonical substance view** — the single projection every consumer embeds, hashes, and lexically indexes. Drops frontmatter, rewrites `[[wikilinks]]` to surface text, cuts **no-embed blocks** (#39), pins byte-level details so the view is machine-identical. `content_hash` fingerprints that same view, which is what lets the pipeline skip a cosmetic re-embed. Also `estimate_tokens` — the embed budget is tokens, not lines. |
| `marked_block.py` | Shared begin/end marked-block logic over arbitrary markers (`related_auto:`, the README region, the global-memory nudge, no-embed). **Write** side (`splice_block`/`remove_block`) raises on a half-marked document; **read** side (`remove_all_blocks`/`unpaired_markers`) is total, so a projection never throws on the malformed input it describes. |
| `embed_staged.py` | Pre-commit helper — (re)generate `.embed.json` sidecars for **staged** PARA notes. Also prints the two **advisory** embed-input warnings (unpaired no-embed marker; canonical view near the context budget) — never blocking, since a hook that refuses a note is a note the user cannot save. |
...
| `tools/update_brain.py` | **Lifecycle (G4)** — non-destructively upgrade an existing brain's tooling from `template/` (dry-run; `--apply` commits). Preserves `vault/`/`data/`/`config/` + history, and the user's space outside the `README.md`/`CLAUDE.md` managed markers, which it **splices** rather than clobbers (#9, #40). `--adopt` brings either file, when marker-less, under management — keeping the old file verbatim below the block. Writes exactly one file inside the vault — `vault/templates/new-note.md` (`VAULT_OWNED`), devkit-owned but located there for Obsidian's Templates plugin, and the carrier of the gate-9 note gate. |
...
| `tools/check_claude_block.py` | **Gate 16** — an *upgraded* brain must receive documentation updates, not just code (#40). Drives the paths a from-scratch check cannot reach: splice preserves user space, is idempotent, a marker-less `CLAUDE.md` **or README** is **named** (never silently skipped, and the run does not sign off "already up to date"), `--adopt` loses not a byte and leaves the brain genuinely managed, half-marked is refused, and the note template that would break gate 9 after an upgrade is refreshed while every real note beside it stays untouched. |
| `tools/check_embed_excluded.py` | **Gate 15** — the emitted no-embed block (#39) must be cut from the embed input **and** from the content hash. Same view or the two disagree: a redrawn diagram would re-embed the note *and* read stale forever. Also pins the **unmarked** path against a literal digest, since drift there restamps every hash in every existing brain. |
