...
1a. **Write for a reader unfamiliar with the project — carry enough top-level context to be self-contained.** `ai-project-status` summarizes these commits **across the whole portfolio**, so a low-level, jargon-only message summarizes poorly. Name the capability in plain language and say *what it does for the brain/user*, not just the internal mechanic. Lead with the outcome, then the detail: prefer `feat(embed): skip re-embedding notes whose content is unchanged (faster, less churn)` over `add content_hash no-op gate to write_sidecar`. Assume the reader knows neither the file names nor the internal shorthand; a bare `lazy-import db in autolink` or `wire gate 4/7` is too esoteric on its own. This applies to the title **and** `[Context]`/`[Impact]` — spell out the why and the effect in terms a newcomer understands.
