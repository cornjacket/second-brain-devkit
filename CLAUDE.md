...
- PDF ingestion — chunk-and-embed long documents (task #7, **design/M0 written, not started**): breaks "one note = one vector" (many chunk-vectors per source); bolt-on schema, PDF git-ignored in the vault (Git-LFS later), `pypdf` optional dep, folder-first `add_pdf` with a chat-or-elicitation selection UI (Desktop Chat lacks elicitation — issue #56243 — so chat is the baseline) → [docs/pdf-ingestion.md](docs/pdf-ingestion.md)
