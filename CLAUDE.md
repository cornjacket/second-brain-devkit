...
- Desktop e2e against your real brain — disposable-branch setup/teardown so the #33 suite runs against `~/second-brain` with no Desktop reconfig, then reverts byte-identical (task #34, built: `desktop-e2e/setup.sh`+`teardown.sh`; smoke-tested end-to-end; human Desktop pass still to run) → [docs/desktop-e2e-disposable-branch.md](docs/desktop-e2e-disposable-branch.md)
