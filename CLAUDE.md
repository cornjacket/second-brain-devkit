...
- PDF ingestion — interactive selection via MCP elicitation (task #7 follow-up, **built**): `add_pdf_guided` MCP tool walks folder → PDF → PARA as client-rendered elicitation forms, falling back to the `list_inbox_pdfs`/`add_pdf` chat flow at runtime otherwise. **Which clients actually render the forms is unsettled — see the doc, not this line.** Task #40 (done) made a fallback say *why*, and the doc opens with a **▶ NEXT ACTION** block: a two-minute human-run diagnosis that resolves it. **If asked "what's next", read that block and relay it.** → [docs/pdf-elicitation.md](docs/pdf-elicitation.md)
