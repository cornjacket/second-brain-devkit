...
- Encrypted notes at rest — opt-in encryption of a brain's committed form, **bodies and
  filenames both**, while the working tree stays plaintext `.md` (task #42, built; CI gates
  17 `tools/check_encryption.py`, 18 `tools/check_content_classification.py`, 19
  `tools/check_encrypted_roundtrip.py`; twin harness `tools/mirror_brain.py`). What is
  content follows `update_brain.py`'s own preserve rule rather than a list. Encryption
...
- Embed-budget audit — `doctor.py` names the notes nearing the embed ceiling (task #57, built; gates 11 + 20 extended). A note past the budget **fails to embed** rather than degrading, and `embed_staged.py` only ever spoke about the note in front of it, so a vault approached the ceiling one untouched file at a time. The ceiling is **2048, measured** (binary search: 2031 words embed, 2046 fail) — `ollama show` advertises both 2048 and an 8192 `num_ctx`, and the embeddings endpoint honours the architecture. Counted over `canonical_body`, so fences and `embed: false` cost nothing. Adds `Report.needs_edit` so the summary stops promising `--repair` for the one problem class it cannot fix. → [docs/embed-budget-audit.md](docs/embed-budget-audit.md)
- Dashboard index — a brain reports its own state at `vault/dashboard-index.md` (task #56, built; gate 8 extended in `tools/check_claude_block.py`). **Similarity is not state**: folders, tags, embeddings and wikilinks all relate notes and none of them knows a checkbox is unticked. Placed at the vault **root, outside every PARA root**, so it is unembedded *by construction* rather than by an `embed: false` flag that can be forgotten. Contract is **point, don't copy** — a mirrored checklist starts lying the first time the wrong copy is ticked. Ships a third `update_brain` behaviour, `VAULT_SEEDED` (create if absent, never overwrite), because owning it would clobber live user status and preserving it would never deliver it; encrypted brains are skipped, which gate 18 caught rather than foresight. → [docs/dashboard-index.md](docs/dashboard-index.md)
