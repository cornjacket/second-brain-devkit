...
- Managed blocks — devkit region + user space in the brain's `README.md` (task #9) **and `CLAUDE.md`** (task #40, built; CI gate 16 `tools/check_claude_block.py`). `update_brain.py` splices both; a marker-less `CLAUDE.md` (a pre-#40 brain) is reported loudly and adoptable via `--adopt`, because the only reader who would notice stale directives is an agent, which has no other copy to compare against. → [docs/readme-managed-block.md](docs/readme-managed-block.md)
