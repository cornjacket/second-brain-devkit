...
- Plugin packaging — one Claude Code plugin vs. the skill + MCP two-step (task #23, research/notes-only): **verdict — a plugin can't collapse the two-step** because its bundled MCP server serves only the CLI + Desktop Code tab, not the Desktop **Chat** tab where a brain is used; the real Desktop-onboarding win is a `.mcpb`/Connector (separate system, follow-up) → [docs/plugin-packaging.md](docs/plugin-packaging.md)
