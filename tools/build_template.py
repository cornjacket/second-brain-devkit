...
def clean_claude(text: str) -> str:
    ...
    # 4. Strip any *foreign* managed block — a region some other tool splices into the
    #    golden because the golden is a live dev repo. Named by shape, not by tool: this
    #    transform used to name one tracker specifically, and when that tracker was
    #    replaced (2026-08-04) the anchor vanished and the build died. Anything that is
    #    not `second-brain:` is by construction dev-process, so it is matched generically
    #    and a golden with no such block is simply left alone (`count=0` is fine here —
    #    the *absence* of dev-process content is never a drift worth failing on).
    text = re.sub(
        r"\n*<!-- (?!second-brain:)([A-Za-z0-9._-]+):begin -->.*?<!-- \1:end -->\n*",
        "\n",
        text,
        flags=re.DOTALL,
    )
    # 5. No blank line before the managed-block END marker. `splice_block` regenerates
    #    that region as `…body…\nEND`, so a blank line here would stop the template being
    #    a fixed point of its own splice — update_brain would report a pristine, freshly
    #    generated brain as CHANGED forever. This used to be a side effect of removing the
    #    block above; it is now stated as the rule it always was. Caught by gate 8.
    text = sub_once(
        text,
        r"\n+(<!-- END generated by second-brain-devkit -->)",
        r"\n\1",
        "CLAUDE.md managed-block END marker",
    )
    return text
