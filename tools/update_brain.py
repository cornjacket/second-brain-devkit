...
# Both files are spliced identically WHEN MARKED. They differ in one place only: what to do
# with a brain whose copy predates the markers entirely (task #40).
#
#   README.md  — skip. It is a presentation file a user may legitimately take full ownership
#                of, and a stale README misleads a human who can see it is stale.
#   CLAUDE.md  — offer adoption (`--adopt`). It is not prose *about* the brain, it is the
#                operating instructions an agent follows, and the reader who would notice it
#                is stale is the one entity that cannot: the agent has no other copy to
#                compare against. Silently skipping it forever is the #40 bug — a brain that
#                takes every code fix and none of the directives that explain them.
#
# Adoption still requires the flag: a personalised CLAUDE.md cannot be told apart from an
# untouched one, so the tool asks rather than guesses.
ADOPTABLE = (CLAUDE,)
...
def _managed_target(rel: str, brain: Path, template_text: str,
    ...
    if not present:
        ...
        if not adopt:
            return ("skip", None,
                    "no devkit markers — predates managed CLAUDE.md, so devkit directives "
                    "(and any future ones) will NOT reach this brain. Re-run with --adopt to "
                    "bring it under management; your current file is kept below the block")
    ...
    if not (new or changed):
        if unmanaged:
            print(f"\n⚠  tooling is up to date, but {', '.join(unmanaged)} is NOT under "
                  "management —\n   this brain will keep missing devkit directives until you "
