...
from test_corpus import install_corpus  # noqa: E402
...
def create_second_brain(
    ...
    ``seed_test_corpus`` (task #16) additionally copies the devkit's test corpus into
    the new brain and commits it — a one-shot populated test brain. It runs *after*
    hooks are wired so the notes embed + hydrate on that commit (needs a working
    embedder), and *before* any push so the corpus is included.
    """
