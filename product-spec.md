...
pipeline stage. It is the **design-internal** definition of a brain and lives
here in the devkit (the generator + system home) — it is deliberately **not**
shipped into a generated brain (see [OQ-4](open-questions.md#oq-4)); a brain's
users are served by its operational `README.md` instead.

The generator produces a repo that conforms to this spec, and validates its
output by diffing against the golden reference
([`../second-brain-test`](https://github.com/cornjacket/second-brain-test)). For
the wider picture — the three-repo system, knowledge flow, generation modes, and
lifecycle — see the system spec ([SPEC.md](SPEC.md)).
...
roots is embedded; repo meta files (`README.md`, `CLAUDE.md`, `scripts/`) are not.
...
brain user must never be required to adopt that tooling. (This independence is a
property of the *product*; it is stated here, in the devkit-owned spec, and is
deliberately never named inside a generated brain — see
[OQ-4](open-questions.md#oq-4)/§7.)
...
A generated brain (the golden reference mirrors this, minus its own dev-process
files):
...
README.md            # operational guide (record / query / setup) + provenance -> devkit
```
...
- **Not in a brain:** this design spec (lives here in the devkit, OQ-4), and the
  golden's own dev-process files (`PLAN.md`, `tasks/`, `daily-plan.md`, …).
