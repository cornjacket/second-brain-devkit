...
**Focus:** 07-09 was all test-corpus: **#16** shipped the seed/teardown utility (100 notes,
install/remove + `--seed-test-corpus`), then **#17** raised its clustering — rewrote all 100 note
bodies ~3× longer and topic-anchored (purity@1 69%→79%, @5 55%→75%; 84% under the `clustering:`
prefix), captured the `clustering:`-prefix reasoning in the retrieval docs. Both committed, CI 7/7.
Today: **finally close the managed-block thread with #9**, then the light #18 checkpoint and start
the diverse corpus #15.
...
```
 ...
 thu 07-09 ✅ #16 test-corpus utility · #17 clustering improvement (purity 69→79% / 84%)
                              ▼
 fri 07-10  ▶▶ build #9 README managed block (via #10 helper)
